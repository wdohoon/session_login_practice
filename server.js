...
app.post("/", (req, res) => {
  ...
  const { userId, userPassword } = req.body; // 구조분해 할당
  const {} = req.body;
...
app.delete("/", (req, res) => {
  ...
  req.session.destroy((err) => {
    ...
    res.clearCookie("session_id"); // 세션 ID 쿠키 삭제
    res.send("🧹세션 삭제 완료");
  });
});
